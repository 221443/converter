  ...
  return (
    <div className="w-full max-w-2xl bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-8 space-y-6 mx-auto">
      <div className="text-center">
        <h1 className="text-3xl md:text-4xl font-bold text-indigo-600 dark:text-indigo-400">Image & PDF Converter</h1>
        <p className="text-gray-600 dark:text-gray-400 mt-2">Convert files right in your browser. Fast, private, and secure.</p>
      </div>
      <div>
        <label
          htmlFor="image-input"
        ...
        >
          <svg className="w-12 h-12 text-gray-400 dark:text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
            ...
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M10 9v6m3-3H7"></path>
          </svg>
          <p className="mt-2 text-sm text-gray-500 dark:text-gray-400">
            <span className="font-semibold">Click to upload</span> or drag and drop
          </p>
          <p className="text-xs text-gray-500 dark:text-gray-400">Images (PNG, JPG, SVG, etc.) and PDF files</p>
        </label>
        <input type="file" id="image-input" accept="image/*,application/pdf,image/svg+xml" className="hidden" multiple onChange={handleFileSelect} />
      </div>
        ...
        <div className="space-y-6 visible-section">
          <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-4 max-h-48 overflow-y-auto bg-gray-50 dark:bg-gray-700/50 p-3 rounded-lg">
            {previews.map((src, idx) => (
              <img key={idx} src={src} alt={`preview-${idx}`} className="w-full h-full object-cover rounded-md shadow-sm" />
            ))}
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-end">
            ...
            {(format === "jpeg" || format === "webp") && (
              <div>
                <label htmlFor="quality-slider" className="block text-sm font-medium text-gray-700 dark:text-gray-300">
      ...
      {output && (
        <div className="text-center p-4 bg-green-50 dark:bg-green-900/50 border border-green-200 dark:border-green-700 rounded-lg visible-section">
          <h3 className="text-lg font-semibold text-green-800 dark:text-green-200">Conversion Complete!</h3>
